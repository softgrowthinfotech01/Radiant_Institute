...
SELECT 
    ...
    COALESCE(
        JSON_ARRAYAGG(gallery_images.image),
        JSON_ARRAY()
    ) AS images
FROM galleries
LEFT JOIN gallery_images 
    ON galleries.id = gallery_images.gallery_id
    AND gallery_images.image IS NOT NULL
    AND gallery_images.image != ''
     ...
     <?php foreach ($images as $extra_activ): ?>
      ...
      <div class="gallery-card group" data-aos="zoom-in">
        <div class="relative overflow-hidden rounded-2xl shadow-2xl cursor-pointer">
          ...
          <img src="../src/uploads/gallery/<?= $extra_activ ?>"
